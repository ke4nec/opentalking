...
“视频创作”用于生成一段可下载的数字人口播视频。它和“实时对话”不同：用户先选择数字人形象、音频来源和生成模型，服务端完成离线生成后，把结果保存到导出视频资产库。

如果你把它叫做“视频制作”，可以理解为同一个入口：当前 WebUI 顶部标签显示为“视频创作”。
...
说明当前后端没有暴露对应模型。检查启动命令中的 `--backend`、`--model`，或确认 OmniRT / local runtime 是否已启动。
